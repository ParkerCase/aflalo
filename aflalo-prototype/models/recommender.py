...
Describe the garment's main/dominant color as it appears in the image. Only these three words, nothing else. Example: solid,silk,cream"""
    ...
    def _build_upload_similarity_reason(self, base_features, candidate_features, candidate_product=None, upload_color_name=None):
        ...
        if base_rgb is not None and cand_rgb is not None:
            color_distance = float(np.linalg.norm(np.asarray(base_rgb, dtype=np.float64).ravel()[:3] - np.asarray(cand_rgb, dtype=np.float64).ravel()[:3]))
        texture_gap = abs(base_tex - cand_tex)
        brightness_gap = abs(base_bri - cand_bri)
        ...
        name = str(cand.get("name", "")).strip() or "this piece"
        color = str(cand.get("color", "")).strip()
        descriptor = self._candidate_similarity_descriptor(cand)
        ...
        elif min(base_sat, cand_sat) < 0.18:
    ...
    def find_uploaded_similar_items(self, uploaded_image, n=3, item_type_hint="Top"):
        """Find visually similar AFLALO products for an uploaded closet image (Closest AFLALO Match).
        Flow: analyze_uploaded_image → filter candidates by item_type_hint → score with _score_feature_pair
        → apply light/dark penalty (down-rank black for light uploads, ivory for dark) → build per-item
        reason via _build_upload_similarity_reason (color, texture, lightness; upload color in copy)
        → return top n by score then price."""
        if self.products is None:
            self.load_data()
