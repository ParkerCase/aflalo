// FIXED Migration: Insert ChatGPT's Comprehensive Strain Database
...
export async function runStrainMigrationFixed() {
  console.log('🌿 Starting FIXED BFF Strain Database Migration...');
  ...
  try {
    ...
    try {
      await pool.query('ALTER TABLE strains ADD CONSTRAINT strains_name_unique UNIQUE (name)');
      console.log('✅ Added unique constraint on strain name');
    } catch (constraintError) {
      console.log('ℹ️  Unique constraint already exists or adding failed - continuing...');
    }

    // Clear existing strains to start fresh (optional)
    const clearResult = await pool.query('DELETE FROM strains WHERE source = $1', ['ChatGPT Cannabis Expert + Leafly Data']);
    console.log(`🗑️  Cleared ${clearResult.rowCount} existing ChatGPT strains`);
    ...
    for (const chatGPTStrain of STRAIN_DB_MASTER) {
      try {
        ...
        // Insert strain with upsert logic
        ...
        if (result.rowCount && result.rowCount > 0) {
          insertedCount++;
          console.log(`✅ Inserted/Updated: ${transformedStrain.name} (${transformedStrain.type})`);
        }
        
      } catch (insertError) {
        errorCount++;
        console.error(`❌ Error inserting ${chatGPTStrain.name}:`, insertError);
      }
    }

    // Verify final count
    const finalCount = await pool.query('SELECT COUNT(*) as count FROM strains');
    const chatgptCount = await pool.query('SELECT COUNT(*) as count FROM strains WHERE source = $1', ['ChatGPT Cannabis Expert + Leafly Data']);
    
    await pool.end();

    console.log('\n🎉 FIXED Migration Complete!');
    console.log(`📊 Strains inserted/updated: ${insertedCount}`);
    console.log(`⚠️  Errors: ${errorCount}`);
    console.log(`🌿 ChatGPT strains in database: ${chatgptCount.rows[0].count}`);
    ...
    return {
      ...
      errors: errorCount,
      chatgptStrains: parseInt(chatgptCount.rows[0].count),
      total: parseInt(finalCount.rows[0].count)
    };
